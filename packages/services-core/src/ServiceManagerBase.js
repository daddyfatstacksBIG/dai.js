...
class ServiceManagerBase {
   ...
   */
  initialize(settings) {
    // If our current state is preceding the INITIALIZING state, we need to set
    // up initialization
   ...
   */
  authenticate() {
    // Public and Local Services are 'authenticated' whenever they are connected
    // (/initialized)
  ...
  /**
   * This is called by the service implementation object, which is the only
   * authority with knowledge on the true connection status. This will e.g. get
   * called when a ping signal is no longer returned by the back-end and the
  ...
  /**
   * This is called by the service implementation object, which is the only
   * authority with knowledge on the true authentication status. This will e.g.
   * get called when an authentication token expires or a user logs out, and the
   * service implementation object concludes that we are therefore no longer
   * authenticated.
   *
   * @private
   */
