...
export default function standardizeConfig(role, config, resolver) {
  ...
  case 'boolean':
    className =
        resolver ? resolveNameForBoolean(role, config, resolver) : config;
    break;
  default:
...
function resolveNameForBoolean(role, bool, {defaults, disabled}) {
  ...
  if (bool) {
    name = defaults[role];
    if (!name)
      throw new Error(`The "${role}" service has no default`);
  } else {
    name = disabled[role];
    if (!name)
      throw new Error(`The "${role}" service cannot be disabled`);
  }
  return name;
}
