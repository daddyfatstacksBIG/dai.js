...
test('should reject invalid transition maps', () => {
  ...
  expect(() => new StateMachine(ServiceState.CREATED, 'not an object'))
      .toThrow();
  expect(() => new StateMachine('MISSING_INITIAL_STATE', getTransitionMap()))
      .toThrow();
});
...
test('inState should only return true for the current state', () => {
  ...
  expect(buildStateMachine().inState([ ServiceState.CREATED ])).toBe(true);
  expect(buildStateMachine().inState([
    ServiceState.OFFLINE, ServiceState.CREATED
  ])).toBe(true);
...
test('should throw an error when trying to set an illegal state', () => {
  expect(() => buildStateMachine().transitionTo('NOT_AN_EXISTING_STATE'))
      .toThrow(IllegalStateError.Error);
});

test('should throw an error when trying to do an illegal state transition',
     () => {
       expect(() => buildStateMachine().transitionTo('OFFLINE'))
           .toThrow(IllegalStateError.Error);
     });
...
test(
    'should throw an error when asserting a state that is not the current state',
    () => {
      expect(() => buildStateMachine().assertState('OFFLINE'))
          .toThrow(IllegalStateError.Error);
      expect(() => buildStateMachine().assertState('OFFLINE', 'myOperation'))
          .toThrow(IllegalStateError.Error);
    });
