...
test('constructor() should reject invalid type names', () => {
  expect(() => new ServiceBase('INVALID_TYPE', serviceName))
      .toThrow('Invalid ServiceType: INVALID_TYPE');
});
...
test(
    'initialize(), connect(), authenticate() should throw when called directly instead of by the manager',
    done => {
...
test('manager() should have the right service type, name, and dependencies',
     () => {
       const s = {
         LOCAL :
             new ServiceBase(ServiceType.LOCAL, 'LocalService', [ 'L1', 'L2' ]),
...
test(
    'manager().initialize() should call the initialize() method of the service and its dependencies',
    done => {
      let checkpoint = 0;
      const dep = new ServiceBase(ServiceType.LOCAL, 'dep'),
            svc = new ServiceBase(ServiceType.LOCAL, 'svc', [ 'dep' ]);
      ...
      svc.manager().inject('dep', dep).initialize().then(() => {
...
test(
    'manager().connect() should call the connect() method of the service and its dependencies',
    done => {
      let checkpoint = 0;
      ...
      svc.manager().inject('dep', dep).connect().then(() => {
        expect(dep.manager().state()).toBe(ServiceState.READY);
        expect(svc.manager().state()).toBe(ServiceState.READY);
...
test(
    'manager().authenticate() should call the authenticate() method of the service and its dependencies',
    done => {
      let checkpoint = 0;
      ...
      svc.manager().inject('dep', dep).authenticate().then(() => {
        expect(dep.manager().state()).toBe(ServiceState.READY);
        expect(svc.manager().state()).toBe(ServiceState.READY);
...
test(
    'disconnecting a dependency should disconnect the dependency and all its depending services',
    done => {
      const dep = new ServiceBase(ServiceType.PUBLIC, 'dep'),
            svc = new ServiceBase(ServiceType.PUBLIC, 'svc', [ 'dep' ]),
      ...
      Promise.all([ svc.manager().connect(), svc2.manager().connect() ])
          .then(() => {
            expect(dep.manager().state()).toBe(ServiceState.READY);
            expect(svc.manager().state()).toBe(ServiceState.READY);
            ...
            // Check that reconnecting one service will reconnect the
            // dependency, but not the second depending service
            svc.manager().connect().then(() => {
              expect(dep.manager().state()).toBe(ServiceState.READY);
              expect(svc.manager().state()).toBe(ServiceState.READY);
              expect(svc2.manager().state()).toBe(ServiceState.OFFLINE);
...
test(
    'deauthenticating a dependency should deauthenticate the dependency and all its depending services',
    done => {
      const dep = new ServiceBase(ServiceType.PRIVATE, 'dep'),
            svc = new ServiceBase(ServiceType.PRIVATE, 'svc', [ 'dep' ]),
      ...
      Promise
          .all([ svc.manager().authenticate(), svc2.manager().authenticate() ])
          .then(() => {
            expect(dep.manager().state()).toBe(ServiceState.READY);
            expect(svc.manager().state()).toBe(ServiceState.READY);
            expect(svc2.manager().state()).toBe(ServiceState.READY);
...
class DummyService extends ServiceBase {
  ...
  initialize(settings) { this._type = settings.type; }
}

test(
    'settings defined on ServiceManager need to be passed to initialize() correctly',
    done => {
      const s = new DummyService('DummyService');
      s.manager().settings({type : 1}).initialize().then(() => {
