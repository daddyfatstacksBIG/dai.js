import Container,
{ExtractedServiceError, InvalidServiceError, orderServices,
 ServiceAlreadyRegisteredError, ServiceNotFoundError} from '../src/Container';
...
const serviceC =
    new ServiceManager('ServiceC', [ 'ServiceA', 'ServiceB' ]).createService();
...
test('register() should throw when registering an invalid service', () => {
  ...
  expect(() => new Container({name : 'NotAService'}).register())
      .toThrow(InvalidServiceError.Error);
  expect(() => new Container('NotAService').register())
      .toThrow(InvalidServiceError.Error);
});
...
test('service() should by default throw when a service is not found', () => {
  expect(() => new Container().service('IDontExist'))
      .toThrow(ServiceNotFoundError.Error);
});

test('service() should throw when trying to access an extracted service',
     () => {
       expect(() => new Container().service('exchange'))
           .toThrow(ExtractedServiceError.Error);
     });

test(
    'service() should not throw when a service is not found and throwIfMissing = false',
    () => {
      expect(new Container().service('IDontExist', false)).toBe(undefined);
    });

test(
    'service() should always throw when a service is requested but no name is provided',
    () => {
      expect(() => new Container().service())
          .toThrow(ServiceNotFoundError.Error);
      expect(() => new Container().service('', false))
          .toThrow(ServiceNotFoundError.Error);
    });
...
test('injectDependencies() throws when missing dependencies', () => {
  expect(() => new Container().register(serviceB).injectDependencies())
      .toThrow(ServiceNotFoundError.Error);
  expect(() => new Container().register(serviceC).injectDependencies())
      .toThrow(ServiceNotFoundError.Error);
});
...
test('orderServices() throws on dependency loops', () => {
  expect(() => { orderServices([ serviceG ]); }).toThrow();

  expect(() => { orderServices([ serviceE, serviceF ]); }).toThrow();

  const badServiceA =
      new ServiceManager('ServiceA', [ 'ServiceD' ]).createService();
  expect(() => {
