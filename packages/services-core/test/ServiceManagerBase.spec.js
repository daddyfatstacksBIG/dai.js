import ServiceManagerBase from '../src/ServiceManagerBase';
import ServiceState from '../src/ServiceState';
import ServiceType from '../src/ServiceType';

test('constructor() should throw on invalid init function', () => {
  expect(() => new ServiceManagerBase(''))
      .toThrow('Invalid argument init: not a function or null.');
  expect(() => new ServiceManagerBase(false))
      .toThrow('Invalid argument init: not a function or null.');
});

test('constructor() should throw on invalid connect function', () => {
  expect(() => new ServiceManagerBase(null, ''))
      .toThrow('Invalid argument connect: not a function or null.');
  expect(() => new ServiceManagerBase(null, false))
      .toThrow('Invalid argument connect: not a function or null.');
});

test('constructor() should throw on invalid auth function', () => {
  expect(() => new ServiceManagerBase(null, null, ''))
      .toThrow('Invalid argument auth: not a function or null.');
  expect(() => new ServiceManagerBase(null, null, false))
      .toThrow('Invalid argument auth: not a function or null.');
});

test(
    'constructor() should correctly set default lifecycle functions when they are omitted',
    () => {
      expect.assertions(3);
...
test('constructor() should correctly determine the service type', () => {
  ...
  expect(new ServiceManagerBase(() => {}).type()).toBe(ServiceType.LOCAL);
  expect(new ServiceManagerBase(null, () => {}).type())
      .toBe(ServiceType.PUBLIC);
  expect(new ServiceManagerBase(null, null, () => {}).type())
      .toBe(ServiceType.PRIVATE);
});

test(
    'initialize() should call and wait for the provided init function when INITIALIZING',
    () => {
      let s = {}, called = false;
      expect.assertions(2);
...
test(
    'initialize() should correctly transition to READY/OFFLINE after successfully INITIALIZING',
    () => {
      expect.assertions(2);
...
test(
    'initialize() should correctly revert back to CREATED after unsuccessfully INITIALIZING',
    async () => {
      expect.assertions(2);
...
test(
    'initialize() should call onInitialized handlers and reflect state through the is* methods',
    done => {
      const s = {};
      let inits = 0;
      ...
      Promise
          .all([
            s.local.initialize(), s.public.initialize(), s.private.initialize()
          ])
          .then(() => {
            expect(inits).toBe(6);
            done();
...
test(
    'initialize() should return the existing init promise when called twice, and should call init only once',
    () => {
      let called = 0;
      const s = new ServiceManagerBase(() => { called++; });
...
test(
    'initialize() should call onReady() of Local Services and reflect state through the is* methods',
    done => {
      const s = {};
      let checkpoints = 0;

      s.local =
          new ServiceManagerBase().onReady(() => checkpoints++).onReady(() => {
            expect(s.local.state()).toBe(ServiceState.READY);
            expect(s.local.isInitialized()).toBe(true);
            expect(s.local.isConnected()).toBe(null);
            expect(s.local.isAuthenticated()).toBe(null);
...
test(
    'connect() should first wait for OFFLINE and then call the provided connect function when CONNECTING',
    done => {
      let counter = 2;
          ...
          () => {
            ...
            output =
                [
                  ...
                  '== init ==', 'INITIALIZING > OFFLINE',
                  'OFFLINE > CONNECTING', '== connect ==', 'CONNECTING > READY'
                ],
            log1 = [], log2 = [],
            service1 = new ServiceManagerBase(() => log1.push('== init =='),
                                              () => log1.push('== connect ==')),
            service2 =
                new ServiceManagerBase(() => { log2.push('== init =='); },
                                       () => { log2.push('== connect =='); });

      service1._state.onStateChanged((ns, os) => log1.push(ns + ' > ' + os));
      service2._state.onStateChanged((ns, os) => log2.push(ns + ' > ' + os));

      service1.initialize().then(() => service1.connect()).then(() => {
...
test(
    'connect() should correctly revert back to OFFLINE after unsuccessfully CONNECTING',
    async () => {
      expect.assertions(2);
      const s = new ServiceManagerBase(
          null, () => { throw new Error('ConnectError'); });
      try {
        await s.connect();
      } catch (err) {
...
test(
    'connect() should call onConnected handlers and reflect state through the is* methods',
    done => {
      const s = {};
      let checkpoints = 0;
...
test(
    'connect() should return the existing connect promise when called twice, and should call connect only once',
    done => {
      ...
      }, s = new ServiceManagerBase(null, () => { called++; });
...
test(
    'connect() should pass a disconnect() function to the handler, allowing it to disconnect and reconnect',
    done => {
      let disconnect = null;
...
test(
    'connect() should call onReady() of Public Services and reflect state through the is* methods',
    done => {
      const s = {};
      let checkpoints = 0;
...
test(
    '_disconnect() should leave the state unchanged if called in an irrelevant state',
    () => {
...
test(
    '_disconnect() should call onDisconnected handlers and reflect state through the is* methods',
    done => {
      const s = {};
...
test(
    'authenticate() should first wait for ONLINE and then call the provided auth function when AUTHENTICATING',
    done => {
      let counter = 2;
          ...
          () => {
            ...
            output =
                [
                  ...
                  '== init ==', 'INITIALIZING > OFFLINE',
                  'OFFLINE > CONNECTING',
                  '== connect ==', 'CONNECTING > ONLINE',
                  'ONLINE > AUTHENTICATING',
                  '== authenticate ==', 'AUTHENTICATING > READY'
                ],
            log1 = [], log2 = [],
            service1 = new ServiceManagerBase(
                () => log1.push('== init =='), () => log1.push('== connect =='),
                () => log1.push('== authenticate ==')),
            service2 = new ServiceManagerBase(
                () => log2.push('== init =='), () => log2.push('== connect =='),
...
test(
    'authenticate() should correctly revert back to ONLINE after unsuccessfully AUTHENTICATING',
    done => {
...
test(
    'authenticate() should call onAuthenticated handlers and reflect state through the is* methods',
    done => {
      const s = {};
      let checkpoints = 0;
      ...
      s.local = new ServiceManagerBase().onAuthenticated(() => {
        throw new Error(
            'onAuthenticated must never be called on a local service');
      });

      s.public = new ServiceManagerBase(null, () => {}).onAuthenticated(() => {
        throw new Error(
            'onAuthenticated must never be called on a public service');
      });
      ...
      Promise
          .all([
            s.local.authenticate(), s.public.authenticate(),
...
test(
    'authenticate() should return the existing authPromise when called twice, and should call auth only once',
    done => {
      let called = 0, counter = 3;
      ...
      }, s = new ServiceManagerBase(null, null, () => { called++; });
...
test(
    'authenticate() should pass a working deauthenticate() function to the handler',
    done => {
...
test(
    'authenticate() should call onReady() of Private Services and reflect state through the is* methods',
    done => {
      const s = {};
      let checkpoints = 0;
...
test(
    '_deauthenticate() should leave the state unchanged if called in an irrelevant state',
    () => {
      const s = new ServiceManagerBase(null, null, () => {});
      expect.assertions(2);
...
test(
    '_deauthenticate() should call onDeauthenticated handlers and reflect state through the is* methods',
    done => {
      const s = {};
      let checkpoints = 0, deauth = null, disconnect = null;

      s.private =
          new ServiceManagerBase(null, d => (disconnect = d), d => (deauth = d))
              .onDeauthenticated(() => checkpoints++)
              .onDeauthenticated(() => {
                expect(s.private.state()).toMatch(/^(ONLINE)|(OFFLINE)$/);
                expect(s.private.isInitialized()).toBe(true);
                expect(s.private.isConnected())
                    .toBe(s.private.state() === ServiceState.ONLINE);
                expect(s.private.isAuthenticated()).toBe(false);
                expect(s.private.isReady()).toBe(false);
...
test(
    'should correctly deauthenticate when disconnecting during the READY state.',
    done => {
...
test('should correctly handle a disconnect in the AUTHENTICATING state.',
     done => {
       ...
       s.service.authenticate()
           ...
           .then(() => {
             // onDeauthenticated should NOT be called, because we were not
             // authenticated yet, when the disconnect happened.
