...
{DependencyNotResolvedError,
 UnknownDependencyError} from '../src/ServiceManager';

const name = 'IAmAService', _ = () => {};

test('should only mark objects with a manager() method as valid', () => {
  expect(ServiceManager.isValidService({
    manager : () => new ServiceManager(name)
  })).toBe(true);
...
test('should throw an error when no name is provided', () => {
  ...
  expect(() => new ServiceManager(''))
      .toThrow('Service name must not be empty.');
  expect(() => new ServiceManager(null))
      .toThrow('Service name must not be empty.');
});
...
test('should throw when injecting an unknown dependency', () => {
  expect(() => new ServiceManager(name, [ 'ValidDependency' ])
                   .inject('InvalidDependency', {}))
      .toThrow(UnknownDependencyError.Error);
});

test('should throw when injecting an invalid service as dependency', () => {
  expect(() => new ServiceManager(name, [
                 'ValidDependency'
               ]).inject('ValidDependency', {
...
test('should return an injected dependency by name', () => {
  ...
  expect(new ServiceManager(name, [ 'ServiceA' ])
...
test('should throw when requesting an unresolved dependency', () => {
  expect(() => new ServiceManager(name, [ 'ServiceA' ]).dependency('ServiceA'))
      .toThrow('Dependency ServiceA of service ' + name + ' is unavailable.');
});

test(
    'should initialize, connect, or authenticate its dependencies before itself',
    done => {
      const log = [],
            dep =
                new ServiceManager('dep', [], () => log.push('DI'),
                                   () => log.push('DC'), () => log.push('DA')),
            svc =
                new ServiceManager('svc', [ 'dep' ], () => log.push('SI'),
                                   () => log.push('SC'), () => log.push('SA'));

      svc.inject('dep', dep.createService()).authenticate().then(() => {
...
test(
    'should build and return a valid service object when calling createService()',
    () => {
      expect(ServiceManager.isValidService(
...
test(
    'should throw when initializing, connecting, or authenticating dependencies before injection',
    () => {
      const serviceA =
          new ServiceManager('ServiceA', [ 'ServiceB', 'ServiceC' ], _, _, _);
      expect(() => serviceA.initializeDependencies())
          .toThrow(DependencyNotResolvedError.Error);
      expect(() => serviceA.connectDependencies())
          .toThrow(DependencyNotResolvedError.Error);
      expect(() => serviceA.authenticateDependencies())
          .toThrow(DependencyNotResolvedError.Error);
    });

test(
    'should resolve to empty array when initializing, connecting or authenticating empty list of dependencies',
    () => {
...
test(
    "should reject with dependency's reason when initializing, connecting or authenticating failing dependency",
    () => {
      const serviceA = new ServiceManager('ServiceA', [], _, _, _);
      ...
      expect.assertions(3);
      serviceB.initializeDependencies().then(
          null, reason => expect(reason).toBe('InitErrorA'));
      serviceB.connectDependencies().then(
          null, reason => expect(reason).toBe('ConnectErrorA'));
      serviceB.authenticateDependencies().then(
          null, reason => expect(reason).toBe('AuthenticationErrorA'));
    });
