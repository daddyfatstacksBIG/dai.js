...
export default class GlobalSettlementCollateralClaims {
  ...
  async check() {
    ...
    const cdpManager =
        this._container.get('smartContract').getContract('CDP_MANAGER_1');
    const vat = this._container.get('smartContract').getContract('MCD_VAT_1');

    const [ids, , ilks] = await this._container.get('smartContract')
                              .getContract('GET_CDPS_1')
                              .getCdpsDesc(cdpManager.address, address);

    const freeCollateral = await Promise.all(ids.map(async (id, i) => {
