...
export const globalSettlement = {
  ...
  afterFlow : fixes => ({
    ...
    fix : async ilk => {
      const ilkAsString = bytesToString(ilk);
      return fixes[ilkAsString] ? RAY.times(fixes[ilkAsString])
                                : new BigNumber(0);
    }
  })
};

export function mockContracts(smartContractService,
                              contractToMockImplementation) {
  const originalGetContract =
      smartContractService.getContract.bind(smartContractService);
  return jest.spyOn(smartContractService, 'getContract')
