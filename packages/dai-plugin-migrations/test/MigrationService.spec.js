...
import GlobalSettlementCollateralClaims from
    '../src/migrations/GlobalSettlementCollateralClaims';
import GlobalSettlementDaiRedeemer from
    '../src/migrations/GlobalSettlementDaiRedeemer';
import GlobalSettlementSavingsDai from
    '../src/migrations/GlobalSettlementSavingsDai';
...
test('can access migration contracts', async () => {
  const managerAddress =
      maker.service('smartContract').getContract('CDP_MANAGER').address;
  const migration = maker.service('smartContract').getContract('MIGRATION');
  const migrationProxyActions =
      maker.service('smartContract').getContract('MIGRATION_PROXY_ACTIONS');
...
test('can fetch a list of all migrations', () => {
  ...
  expect(ids).toEqual(expect.arrayContaining([
    Migrations.SINGLE_TO_MULTI_CDP, Migrations.SAI_TO_DAI,
    Migrations.GLOBAL_SETTLEMENT_SAVINGS_DAI,
    Migrations.GLOBAL_SETTLEMENT_COLLATERAL_CLAIMS,
    Migrations.GLOBAL_SETTLEMENT_DAI_REDEEMER, Migrations.MKR_REDEEMER,
...
test('getting each migration returns a valid migration', () => {
  expect(service.getMigration(Migrations.SINGLE_TO_MULTI_CDP))
      .toBeInstanceOf(SingleToMultiCdp);
  expect(service.getMigration(Migrations.SAI_TO_DAI)).toBeInstanceOf(SaiToDai);
  expect(service.getMigration(Migrations.GLOBAL_SETTLEMENT_SAVINGS_DAI))
      .toBeInstanceOf(GlobalSettlementSavingsDai);
  expect(service.getMigration(Migrations.GLOBAL_SETTLEMENT_COLLATERAL_CLAIMS))
      .toBeInstanceOf(GlobalSettlementCollateralClaims);
  expect(service.getMigration(Migrations.GLOBAL_SETTLEMENT_DAI_REDEEMER))
      .toBeInstanceOf(GlobalSettlementDaiRedeemer);
  expect(service.getMigration(Migrations.MKR_REDEEMER))
      .toBeInstanceOf(MkrRedeemer);
});
