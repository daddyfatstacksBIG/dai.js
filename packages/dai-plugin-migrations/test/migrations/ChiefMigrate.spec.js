...
describe('Chief Migration', () => {
  ...
  beforeEach(async () => { snapshot = await takeSnapshot(maker); });

  afterEach(() => { restoreSnapshot(snapshot, maker); });
  ...
  test(
      'if the account has some MKR locked directly in old chief, return the amount',
      async () => {
        const oldChief =
            maker.service('smartContract').getContractByName('OLD_CHIEF');

        await maker.service('token').getToken(MKR).approveUnlimited(
            oldChief.address);
  ...
  test(
      'if the account has some MKR locked via proxy in old chief, return the amount',
      async () => {
        const oldVoteProxyFactory =
            maker.service('smartContract')
        ...
        // and back to cold
        maker.useAccount('default');
        const voteProxyAddress =
            await migration._getVoteProxyAddress(coldAccount.address);
