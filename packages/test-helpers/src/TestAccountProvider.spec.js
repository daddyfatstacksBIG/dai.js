import {default as provider, TestAccountProvider} from './TestAccountProvider';

test('should reject illegal indices', () => {
  const msg = 'Index must be a natural number between 0 and ' +
...
test('should reject accounts objects', () => {
  expect(() => new TestAccountProvider({addresses : [ 'x' ], keeys : [ 'y' ]}))
      .toThrow('Accounts must be an object with properties addresses and keys');

  expect(() => new TestAccountProvider(
             {addresses : [ 'x', 'y', 'z' ], keys : [ 'x', 'y' ]}))
