...
const serviceRoles = [
  'accounts', 'allowance', 'cdp', 'conversion', 'gas', 'log', 'multicall',
  'nonce', 'price', 'proxy', 'smartContract', 'timer', 'token',
  'transactionManager', 'web3'
];
...
const reservedWords = [
  'accounts', 'overrideMetamask', 'plugins', 'privateKey', 'provider', 'url'
];
...
export default class ConfigFactory {
  ...
  static create(preset, options = {}, resolver) {
    ...
    for (let role of serviceRoles.concat(additionalServices)) {
      ...
      config[role] =
          mergeServiceConfig(role, config[role], options[role], resolver);
    }

    // web3-specific convenience options
    if (config.web3) {
      const web3Settings = config.web3[1] || config.web3;
      if (!web3Settings.provider)
        web3Settings.provider = {};
