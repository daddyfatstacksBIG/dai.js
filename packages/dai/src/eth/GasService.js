...
export default class GasService extends PublicService {
  ...
  initialize(settings) {
    ...
    this._gasStationDataPromise =
        this.disablePrice ? Promise.resolve({}) : this.fetchGasStationData();
  }
  ...
  _setProperties(settings, label) {
    ...
    return map(settings, (value, key) => {
      if (key === 'disable') {
        this['disable' + label.charAt(0).toUpperCase() + label.slice(1)] =
            value;
      } else {
        this[key] = value;
      }
  ...
  async getGasPrice(txSpeed) {
    ...
    const price = this.get('web3')._web3.utils.toWei(
        (gasStationData[speedSetting] / 10).toString(), 'gwei');
  ...
  async estimateGasLimit(transaction) {
    ...
    if (!this.multiplier && !this.absolute) {
      ...
      return Math.min(parseInt(estimate * this.multiplier), this.absolute,
                      blockLimit);
    }
  }
  ...
  get transactionSpeed() { return this._transactionSpeed; }
  ...
  removeMultiplier() { this._multiplier = null; }

  removeAbsolute() { this._absolute = null; }

  removeFallback() { this._fallback = null; }
}
