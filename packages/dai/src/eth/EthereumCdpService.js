...
export default class EthereumCdpService extends PrivateService {
  ...
  constructor(name = 'cdp') {
    super(name, [
      'allowance', 'conversion', 'event', 'price', 'proxy', 'smartContract',
      'token'
    ]);
  ...
  _smartContract() { return this.get('smartContract'); }

  _txMgr() { return this.get('smartContract').get('transactionManager'); }
  ...
  _web3Service() { return this._smartContract().get('web3'); }

  _conversionService() { return this.get('conversion'); }
  ...
  async getCdp(id, dsProxy) {
    ...
    if (dsProxy) {
      assert(owner === dsProxy.toLowerCase(),
  ...
  async _throwIfNotEnoughMkrToWipe(cdpId, amountToWipe, unit = DAI) {
    const enoughMkrToWipe =
        await this.enoughMkrToWipe(cdpId, amountToWipe, unit);
  ...
  async enoughMkrToWipe(cdpId, amountToWipe, unit = DAI) {
    ...
    const ownerAddress = this.get('token').get('web3').currentAddress();
    const [fee, balance, debt] = await Promise.all([
      this.getGovernanceFee(cdpId, MKR), MkrToken.balanceOf(ownerAddress),
      this.getDebtValue(cdpId)
    ]);
    const mkrOwed = dai.div(debt).toBigNumber().times(fee.toBigNumber());
  ...
  openCdp() { return new Cdp(this).transactionObject(); }
  ...
  openProxyCdpLockEthAndDrawDai(amountEth, amountDai, dsProxy = null) {
    return new ProxyCdp(this, dsProxy, null,
                        {lockAndDraw : true, amountEth, amountDai})
        .transactionObject();
  }
  ...
  async shut(cdpId) {
    ...
    await this.get('allowance')
        .requireAllowance(MKR, this._tubContract().address);
    await this.get('allowance')
        .requireAllowance(DAI, this._tubContract().address);
  ...
  async lockEth(cdpId, amount, {unit = ETH, waitForConfirm, promise}) {
    const convert =
        this._conversionService().convertEthToWeth(amount, {unit, promise});
  ...
  async lockWeth(cdpId, amount, {unit = WETH, promise}) {
    ...
    await this._conversionService().convertWethToPeth(weth, {promise});
  ...
  async lockPeth(cdpId, amount, {unit = PETH, promise}) {
    ...
    await this.get('allowance')
        .requireAllowance(PETH, this._tubContract().address, {promise});
    return this._tubContract().lock(hexCdpId, value, {promise});
  }
  ...
  async getCollateralValue(cdpId, unit = ETH) {
    ...
    throw new Error(`Don't know how to get collateral value in ${
  ...
  async getDebtValue(cdpId, unit = DAI) {
    ...
    const tab = await new Promise(
        (resolve, reject) => tub.methods.tab(hexCdpId).call(
  ...
  // updates compound interest calculations for all CDPs.  Used by tests that
  // depend on a fee
  async _drip() { return this._tubContract().drip(); }
  ...
  async getCollateralizationRatio(cdpId) {
    ...
    const [pethPrice, pethCollateral] = await Promise.all([
      this.get('price').getPethPrice(), this.getCollateralValue(cdpId, PETH)
    ]);
    return pethCollateral.times(pethPrice).div(usdDebt).toNumber();
  }
  ...
  async getLiquidationPenalty() {
    ...
    return new BigNumber(value.toString()).dividedBy(RAY).minus(1).toNumber();
  }
  ...
  async getTargetPrice() {
    ...
    const par = await new Promise(
        (resolve, reject) => vox.methods.par().call(
            {}, (err, val) => (err ? reject(err) : resolve(val))));
  ...
  async getSystemCollateralization() {
    ...
    const [totalWethLocked, wethPrice, daiSupply, targetPrice] =
        await Promise.all([
          this._tubContract().pie(), this.get('price').getEthPrice(),
          dai.totalSupply(), this.getTargetPrice()
        ]);

    const totalCollateralValue =
        new BigNumber(totalWethLocked).div(WAD).times(wethPrice.toBigNumber());
  ...
  freeEthProxy(dsProxy, cdpId, amount) {
    ...
    return this._saiProxyTubContract().free(
        this._tubContract().address, hexCdpId, value, {
          dsProxy,
          metadata : {
            action : {
              name : 'free',
  ...
  lockEthProxy(dsProxy, cdpId, amount) {
    ...
    return this._saiProxyTubContract().lock(
        this._tubContract().address, hexCdpId, {
          dsProxy,
          value,
          metadata : {
            action : {
  ...
  lockEthAndDrawDaiProxy(dsProxy, cdpId, amountEth, amountDai) {
    ...
    return this._saiProxyTubContract().lockAndDraw(
        this._tubContract().address, hexCdpId, valueDai, {
          dsProxy,
          value : valueEth,
          metadata : {
            action : {
  ...
  giveProxy(dsProxy, cdpId, newAddress) {
    ...
    return this._saiProxyTubContract().give(
        this._tubContract().address, hexCdpId, newAddress, {
          dsProxy,
          metadata : {
            action :
                {name : 'give', id : cdpId, to : newAddress, proxy : dsProxy}
          }
        });
  }
  ...
  async wipeDaiProxy(dsProxy, cdpId, amount, {useOtc, promise}) {
    ...
    await this.get('allowance').requireAllowance(DAI, dsProxy, {promise});
    ...
    if (!useOtc) {
      ...
      await this.get('allowance').requireAllowance(MKR, dsProxy, {promise});
    }
    ...
    return useOtc
               ? this._saiProxyTubContract()['wipe(address,bytes32,uint256,address)'](
                     this._tubContract().address, hexCdpId, value,
                     this._smartContract().getContractAddressByName(
                         contracts.MAKER_OTC),
                     options)
               : this._saiProxyTubContract()['wipe(address,bytes32,uint256)'](
                     this._tubContract().address, hexCdpId, value, options);
  }
  ...
  async shutProxy(dsProxy, cdpId, {useOtc, promise}) {
    ...
    const options = {
      dsProxy,
      metadata : {
        action : {name : 'close', id : cdpId, otc : useOtc, proxy : dsProxy}
      },
      promise
    };
    ...
    return useOtc
               ...
               : this._saiProxyTubContract()['shut(address,bytes32)'](
                     this._tubContract().address, hexCdpId, options);
  }
}
