...
async function getAccountAddress(subprovider, {offset = 0, address} = {}) {
  ...
  return new Promise(
      (resolve, reject) => subprovider.handleRequest(
          {method : 'eth_accounts', params : [], id : 1}, null, (err, val) => {
            if (err)
              return reject(err);
            if (address) {
              const matchingAddress =
                  val.find(a => a.toLowerCase() === address.toLowerCase());
              assert(matchingAddress, 'No matching address found in provider.');
              resolve(matchingAddress.toLowerCase());
            } else {
              resolve(typeof val[offset] === 'string'
