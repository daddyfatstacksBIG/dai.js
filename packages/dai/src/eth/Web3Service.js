...
export default class Web3Service extends PrivateService {
  ...
  /*
    ...
    0.2x.x. it doesn't resolve until the transaction has a receipt, and throws
    an error if the receipt indicates that the transaction was reverted. the
    setup below emulates the old behavior, because TransactionObject still
  ...
  */
  ...
  get rpcUrl() { return last(this._web3.currentProvider._providers).rpcUrl; }
  ...
  _listenForNewBlocks() {
    if (this.usingWebsockets()) {
      this._newBlocksSubscription =
          this.subscribe('newBlockHeaders')
              .on('data', data => this._updateBlockNumber(data.number));
    } else {
