...
export const LAST_CREATED_CDP_COLLATERAL_VALUE =
    'lastCreatedCdpCollateralValue';

const Φ = BigNumber(
    '1.6180339887498948482045868343656381177203091798057628621354486227052604628189');

const validateArgsCdpId = id =>
    !/^\d+$/.test(id) && 'Invalid cdp id: must be a positive integer';
...
export const tubRum = {
  generate : () =>
      ({id : 'SAI_TUB.rum', contract : 'SAI_TUB', call : [ 'rum()(uint256)' ]}),
...
export const tubCups = {
  generate : id => ({
    ...
    contract : 'SAI_TUB',
    call : [
      'cups(bytes32)(address,uint256,uint256,uint256)', numberToBytes32(id)
    ]
  }),
  validate : {
  ...
  returns : [
    [ CDP_OWNER, bytesToString ], [ CDP_COLLATERAL, fromWei ],
    [ CDP_DEBT, fromWei ], [ CDP_IRE, fromWei ]
  ]
};

export const tubCap = {
  generate : () =>
      ({id : 'SAI_TUB.cap', contract : 'SAI_TUB', call : [ 'cap()(uint256)' ]}),
