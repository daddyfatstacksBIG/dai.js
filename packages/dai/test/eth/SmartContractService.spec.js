...
test('getContract should have proper error checking', async () => {
  ...
  expect(() => service.getContract('NOT_A_CONTRACT'))
      .toThrow('No contract found for "NOT_A_CONTRACT"');
  expect(() => service.getContract(contracts.SAI_TOP))
      .toThrow('Cannot resolve network ID. Are you connected?');
...
test('getContract should return a functioning contract', async () => {
  ...
  expect(gem.toString().toUpperCase())
      .toEqual(service.getContract(tokens.WETH).address.toUpperCase());
});
...
test('define contracts in config with multiple addresses', async () => {
  ...
  expect(() => { service.getContract('mock2'); })
...
test('fallback if addressOverrides does not specify current network',
     async () => {
       const service = buildTestService('smartContract', {
         smartContract :
       ...
       await service.manager().authenticate();
       const addresses = service.getContractAddresses();
       expect(addresses.PROXY_REGISTRY)
           .toEqual(originalAddresses.PROXY_REGISTRY);
     });
