...
function buildDisconnectingService(disconnectAfter = 25) {
  ...
  service.manager().onConnected(() => {
    service.get('timer').createTimer(
        'disconnect', disconnectAfter, false, () => {
...
function buildNetworkChangingService(changeNetworkAfter = 25) {
  ...
  service.manager().onConnected(() => {
    service.get('timer').createTimer(
        'changeNetwork', changeNetworkAfter, false, () => {
...
function buildDeauthenticatingService(deauthenticateAfter = 25) {
  ...
  service.manager().onAuthenticated(() => {
    service.get('timer').createTimer(
        'deauthenticate', deauthenticateAfter, false,
...
function buildAccountChangingService(changeAccountAfter = 25) {
  ...
  service.manager().onAuthenticated(() => {
    service.get('timer').createTimer(
        'changeAccount', changeAccountAfter, false, () => {
          service._web3.eth.getAccounts = cb =>
              cb(undefined, [ '0x123456789' ]); // fake account
        });
  });
  return service;
}
...
function buildTestService(key, statusTimerDelay = 5000) {
  const config = {
    web3 : {statusTimerDelay, transactionSettings : {gasPrice : 1}}
  };
  if (key) {
    config.accounts = {default : {type : 'privateKey', key}};
  }
...
function buildInfuraService(network, privateKey = null) {
  return buildTestServiceCore('web3', {
    web3 : {
      privateKey,
      provider : {type : ProviderType.INFURA, network, infuraProjectId},
...
test('fetch version info on connect', done => {
  ...
  web3.manager().connect().then(() => {
    expect(web3.info().api)
        .toMatch(/^([0-9]+\.)*[0-9]([-][b][e][t][a][.]([0-9])*)*$/);
...
test('throw error on a failure to connect', async () => {
  ...
  service._web3.eth.getNodeInfo =
      cb => { cb(new Error('fake connection failure error')); };
...
test(
    'be authenticated and know default address when private key passed in',
    done => {
      const service = buildTestService(
      ...
      service.manager().authenticate().then(() => {
        ...
        expect(service.currentAddress())
            .toBe('0x717bc9648b627316718fe93f4cd98056e53a8c8d');
        done();
      });
    });
...
// this test needs to be commented out in order to push it to git and have it
// pass the tests need to run this test individually and then disconnect the wifi
// for it to pass
/*
...
test('create a ethersjs object running parallel to web3', done => {
  ...
  service.manager().connect().then(() => {
    ...
    expect(service._web3.currentProvider)
        .toBe(service._ethersProvider._web3Provider);
    done();
  });
});

test('connect to ganache testnet with account 0x16fb9...', done => {
  const service = buildTestService(), expectedAccounts = [
...
test('have ETH in test account', done => {
  ...
  service.manager()
      ...
      .then(balance => {
        expect(Number(service._web3.utils.fromWei(balance)))
            .toBeGreaterThan(50);
        done();
      });
});
...
test('stores confirmed block count from config', async () => {
  const config = {web3 : {confirmedBlockCount : 8}};
