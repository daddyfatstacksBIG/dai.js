...
test('get Ether allowance returns max safe integer', done => {
  ...
  ethereumTokenService.manager()
      .authenticate()
      .then(() => {
        const token = ethereumTokenService.getToken(ETH);
        return token.allowance(TestAccountProvider.nextAddress(),
                               TestAccountProvider.nextAddress());
      })
      .then(allowance => {
        expect(allowance.toString())
            .toEqual(Number.MAX_SAFE_INTEGER.toString());
        done();
      });
});
...
test('ether transfer should move transferValue from sender to receiver',
     done => {
       const ethereumTokenService = buildTestEthereumTokenService(),
             receiver = TestAccountProvider.nextAddress();

       let sender = null, token = null, senderBalance = null,
       ...
       ethereumTokenService.manager()
           ...
           .then(() => {
             ...
             token = ethereumTokenService.getToken(ETH);
             return Promise.all(
                 [ token.balanceOf(sender), token.balanceOf(receiver) ]);
           })
           .then(balances => {
           ...
           .then(balances => {
             ...
             expect(newReceiverBalance.minus(0.1).eq(receiverBalance))
                 .toBeTruthy();
             // sender also pays tx fee, so their balance is lower
             expect(newSenderBalance.plus(0.1).toNumber())
                 .toBeCloseTo(senderBalance.toNumber(), 2);
             done();
           });
     });

test('ether transferFrom should move transferValue from sender to receiver',
     done => {
       const ethereumTokenService = buildTestEthereumTokenService(),
             receiver = TestAccountProvider.nextAddress();
