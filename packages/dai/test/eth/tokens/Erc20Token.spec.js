...
test('get ERC20 (MKR) allowance of address', async () => {
  const allowance = await mkr.allowance(TestAccountProvider.nextAddress(),
...
test('ERC20 transfer should move transferValue from sender to receiver',
     async () => {
...
test('ERC20 transferFrom should move transferValue from sender to receiver',
     async () => {
       await weth.deposit('0.1');
       const senderBalance = await weth.balanceOf(currentAddress);
       const receiverBalance = await weth.balanceOf(testAddress);
