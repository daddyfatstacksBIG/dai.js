...
import ProviderSubprovider from
    'web3-provider-engine/dist/es5/subproviders/provider';
...
test('account with private key string literal in settings', async () => {
  ...
  const service = buildTestService(
      'accounts',
      {accounts : {foo : {type : 'privateKey', key : account.key}}});
...
test('provider account with offset', async () => {
  ...
  const service = buildTestService(
      'accounts', {accounts : {foo : {type : 'provider', offset}}});
  await service.manager().connect();
...
test('provider account with address', async () => {
  const {address} = TestAccountProvider.nextAccount();
  const service = buildTestService(
      'accounts', {accounts : {foo : {type : 'provider', address}}});
  await service.manager().connect();
  expect(service.currentAddress()).toEqual(address);
});
...
test('invalid private keys', async () => {
  ...
  for (const key of badKeys) {
    const service = buildTestService(
        'accounts', {accounts : {foo : {type : 'privateKey', key}}});
    try {
      await service.manager().connect();
    } catch (err) {
...
test('currentAccount', async () => {
  ...
  expect(service.currentAccount())
      .toEqual({name : 'foo', type : 'privateKey', address : a1.address});
});
...
describe('mocking window', () => {
    ...
    mockProvider = {
      sendAsync : ({method}, callback) => {
        if (method === 'eth_accounts') {
          callback(null, {
            result : [ window.web3 && window.web3.eth
  ...
  test('use wrong browser account', async () => {
    ...
    try {
      ...
      expect(err.message)
          .toBe('cannot use a browser account that is not currently selected');
    }
  });
  ...
  test('browser autoSwitch', async () => {
    ...
    const promise = new Promise(
        resolve =>
            service.get('event').on('accounts/CHANGE', jest.fn(eventObj => {
              expect(eventObj.payload.account.address).toEqual('0xf01');
              resolve();
            })));
    window.web3.eth.defaultAccount =
        '0xf01'; // this changes when a user switches accounts
  ...
  test(
      'browserProviderAccountFactory with window.ethereum, user accepts provider',
      async () => {
        window.ethereum = {
          enable :
  ...
  test(
      'browserProviderAccountFactory with window.ethereum, user rejects provider',
      async () => {
        window.ethereum = {
          enable : async () => {
            window.ethereum['sendAsync'] = mockProvider.sendAsync;
