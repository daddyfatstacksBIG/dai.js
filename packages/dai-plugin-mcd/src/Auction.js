...
export default class Auction {
  constructor(ilk, smartContractService) {
    ...
    case 'MKR':
      ...
      break;
    default:
      this.contract = smartContractService.getContract('MCD_FLIP_' +
                                                       ilk.replace(/-/g, '_'));
    }
  }

  // returns time in hours
  async getMaxBidLifetime() { return (await this.contract.ttl()) / 3600; }

  // returns time in days
  async getMaxAuctionDuration() { return (await this.contract.tau()) / 86400; }
  ...
  async getMinBidIncrease() {
    ...
    return new BigNumber(beg).div(WAD).minus(1).toNumber();
  }
}
