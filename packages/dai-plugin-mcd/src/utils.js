...
export function nullIfEmpty(value) { return value === '' ? null : value; }
...
export function parseWeiNumeric(value, denom = 'ether') {
  // fromWei will throw if passing a Bignumber value or string value that
  // results in being an exponent representation of a number when parsed as a
  // number, e.g 1e18. Passing value as a hex value seems to get around this
  return web3Utils.fromWei(BigNumber(value).toString(16), denom);
}

export function numberFromNumeric(value) { return BigNumber(value).toNumber(); }
...
export function isBigNumber(value) { return BigNumber.isBigNumber(value); }

export function isCurrency(value) { return value instanceof Currency; }
