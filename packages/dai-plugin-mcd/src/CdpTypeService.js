...
export default class CdpTypeService extends PublicService {
  constructor(name = CDP_TYPE) { super(name, [ SYSTEM_DATA, QUERY_API ]); }

  initialize(settings = {}) {
    this.settings = settings;
    this.cdpTypes =
        (settings.cdpTypes || []).map(cdpType => new CdpType(this, cdpType, {
  ...
  getCdpType(currency, ilk) {
    const types = this.cdpTypes.filter(
        t => (!currency || t.currency.symbol === currency.symbol) &&
             (!ilk || ilk === t.ilk));
    if (types.length === 1)
      return types[0];

    const label = [
      currency && `currency ${currency.symbol}`, ilk && `ilk ${ilk}`
    ].filter(x => x).join(', ');
  ...
  async resetAllCdpTypes() {
    this.cdpTypes.forEach(
        cdpType => { this.getCdpType(null, cdpType.ilk).reset(); });
  }

  async prefetchAllCdpTypes() {
    await Promise.all(this.cdpTypes.map(
        cdpType => this.getCdpType(null, cdpType.ilk).prefetch()));
  }
  ...
  get totalDebtAllCdpTypes() {
    const debts = this.cdpTypes.map(ilk => { return ilk.totalDebt; });
    return debts.reduce((a, b) => a.plus(b));
  }

  get totalCollateralValueAllCdpTypes() {
    const collateralValues = this.cdpTypes.map(
        ilk => { return ilk.totalCollateral.times(ilk.price); });
    return collateralValues.reduce((a, b) => a.plus(b));
  }

  get totalCollateralizationRatioAllCdpTypes() {
    return math.collateralizationRatio(this.totalCollateralValueAllCdpTypes,
