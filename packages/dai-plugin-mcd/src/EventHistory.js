...
import {bytesToString, numberFromNumeric, parseWeiNumeric} from './utils';
...
const EVENT_VAT_FROB =
    funcSigTopic('frob(bytes32,address,address,address,int256,int256)');
const EVENT_MANAGER_FROB = funcSigTopic('frob(uint256,int256,int256)');

const decodeManagerFrob = data => {
  const sig =
      ethAbi.encodeFunctionSignature('frob(uint256,int256,int256)').slice(2);
  ...
  return {
    ...
    dink : decoded[1],
    dart : decoded[2] // can't be used directly because would need to be scaled
                      // up using vat.ilks[ilk].rate
  };
};
...
export default async function getEventHistory(cdpManager, managedCdp, cache) {
  const MCD_JOIN_DAI =
      cdpManager.get('smartContract').getContractAddress('MCD_JOIN_DAI');
  const MCD_JOIN_SAI =
      cdpManager.get('smartContract').getContractAddress('MCD_JOIN_SAI');
  const CDP_MANAGER =
      cdpManager.get('smartContract').getContractAddress('CDP_MANAGER');
  const MIGRATION =
      cdpManager.get('smartContract').getContractAddress('MIGRATION');
  ...
  const {NewCdp} = cdpManager.get('smartContract')
  ...
  const lookups = [
    {
      request : web3.getPastLogs({
        address : CDP_MANAGER,
        topics : [
          utils.keccak256(utils.toHex(NewCdp.signature)), null, null,
          '0x' + padStart(id.toString(16), 64, '0')
        ],
        fromBlock
      }),
      result : r => r.map(({blockNumber : block, transactionHash : txHash}) => {
        return {type : 'OPEN', order : 0, block, txHash, id, ilk};
      })
    },
    {
      request : web3.getPastLogs({
        address : CDP_MANAGER,
        topics : [
          EVENT_MANAGER_FROB, null, '0x' + padStart(id.toString(16), 64, '0')
        ],
        fromBlock
      }),
    ...
    },
    {
      request : web3.getPastLogs({
        address : MCD_VAT,
        topics : [
          EVENT_VAT_FROB, null, '0x' + padStart(urnHandler.slice(2), 64, '0')
        ],
        fromBlock
      }),
      result : r => r.map(
    ...
    },
    {
      request : web3.getPastLogs({
        address : CDP_MANAGER,
        topics :
            [ EVENT_GIVE, null, '0x' + padStart(id.toString(16), 64, '0') ],
        fromBlock
      }),
      result : r =>
  ...
  cache[id] = (async () => {
    ...
    return orderBy(await Promise.all(
                       flatten(await Promise.all(
                                   results.map((r, i) => lookups[i].result(r))))
                           .filter(r => r !== null)
                           .map(async e => {
                             // eslint-disable-next-line require-atomic-updates
                             e.timestamp =
                                 (await getBlockTimestamp(e.block)).timestamp;
                             return e;
                           })),
                   [ 'block', 'order' ], [ 'desc', 'desc' ])
        .map(e => {
          delete e.order;
          return e;
        });
...
export async function getDsrEventHistory(service, address, cache) {
  const MCD_JOIN_DAI =
      service.get('smartContract').getContractAddress('MCD_JOIN_DAI');
  ...
  const lookups = [
    {
      ...
      result : async r => r.reduce(
          async (acc, {blockNumber : block, transactionHash : txHash}) => {
            ...
            const [{topics : adapterTopics}] =
                adapterJoinEvents.filter(x => x.transactionHash === txHash);
            acc.push({
              type : 'DSR_DEPOSIT',
              order : 0,
              block,
    ...
    },
    {
      ...
      result : async r => r.reduce(
          async (acc, {blockNumber : block, transactionHash : txHash}) => {
            ...
            const [{topics : adapterTopics}] =
                adapterExitEvents.filter(x => x.transactionHash === txHash);
            acc.push({
              type : 'DSR_WITHDRAW',
              order : 0,
              block,
