...
export default class CdpManager extends LocalService {
  constructor(name = CDP_MANAGER) {
    super(name, [
      'smartContract', CDP_TYPE, SYSTEM_DATA, 'accounts', 'proxy', 'token',
      'web3'
    ]);
  ...
  async getIlkForCdp(id) { return bytesToString(await this._manager.ilks(id)); }
  ...
  async open(ilk, {promise, cache = true}) {
    ...
    const op =
        this.proxyActions.open(this._managerAddress, stringToBytes(ilk), proxy,
  ...
  async reclaimCollateral(id, dink, {promise}) {
    ...
    return this.proxyActions.frob(
        this._managerAddress, this.getIdBytes(id), dink.toFixed('wei'), 0,
  ...
  async wipeAll(id, owner, {promise} = {}) {
    ...
    return this.proxyActions.safeWipeAll(
        this._managerAddress, this._adapterAddress('DAI'), this.getIdBytes(id),
        owner, {dsProxy : true, promise, metadata : {id}});
  }
