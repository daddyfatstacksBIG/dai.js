...
export default class CdpManager extends LocalService {
  constructor(name = CDP_MANAGER) {
    super(name, [
      'smartContract', CDP_TYPE, SYSTEM_DATA, QUERY_API, 'accounts', 'proxy',
      'token', 'web3'
    ]);
  ...
  async getCdpIds(proxyAddress, descending = true) {
    ...
    if (!this._getCdpIdsPromises[proxyAddress]) {
      this._getCdpIdsPromises[proxyAddress] =
  ...
  async getCombinedDebtValue(proxyAddress, descending = true) {
    ...
    const debts = await Promise.all(ids.map(c => {
  ...
  async getCombinedEventHistory(proxyAddress) {
    ...
    const ilksAndUrns = await Promise.all(cdpIds.map(async c => {
  ...
  async open(ilk, {promise, cache = true}) {
    ...
    const op = this.proxyActions.open(this._managerAddress, stringToBytes(ilk),
                                      proxy, {dsProxy : true, promise});
  ...
  async reclaimCollateral(id, dink, {promise}) {
    ...
    return this.proxyActions.frob(this._managerAddress, this.getIdBytes(id),
                                  dink.toFixed('wei'), 0,
  ...
  @tracksTransactions
  async openLockAndDraw(ilk, lockAmount, drawAmount, {promise, cache = true}) {
    const type = this.get(CDP_TYPE).getCdpType(lockAmount.type, ilk);
    const op =
        this.lockAndDraw(null, type.ilk, lockAmount, drawAmount, {promise});
  ...
  async lockAndDraw(id, ilk, lockAmount, drawAmount = MDAI(0), {promise}) {
    ...
    assert(lockAmount instanceof Currency,
    ...
    const args = [
      this._managerAddress, jugAddress, this._adapterAddress(ilk),
      this._adapterAddress('DAI'), id || stringToBytes(ilk),
      !isEth && lockAmount.toFixed(this._precision(lockAmount)),
      drawAmount.toFixed('wei'),
      {dsProxy : true, value : isEth ? lockAmount.toFixed('wei') : 0, promise}
    ].filter(x => x);
    ...
    if (method === 'openLockGNTAndDraw')
      await this.get('token').getToken('GNT').transfer(proxyAddress,
                                                       lockAmount);
    // Transfers to bag if locking GNT in existing CDP
    if (id && isGnt)
      await transferToBag(lockAmount, proxyAddress, this);
  ...
  async lock(id, ilk, lockAmount, owner, {promise}) {
    if (!owner)
      owner = await this.getOwner(id);
    const proxyAddress = await this.get('proxy').ensureProxy({promise});
    const isEth = ETH.isInstance(lockAmount);
    const isGnt = GNT.isInstance(lockAmount);
    const method = `safeLock${isEth ? 'ETH' : 'Gem'}`;
    const args = [
      this._managerAddress, this._adapterAddress(ilk), id,
      !isEth && lockAmount.toFixed(this._precision(lockAmount)), owner,
      {dsProxy : true, value : isEth ? lockAmount.toFixed('wei') : 0, promise}
    ].filter(x => x);
    ...
    // Indicates if gem supports transferFrom
    if (!isEth)
      args.splice(-2, 0, !GNT.isInstance(lockAmount));
  ...
  wipeAndFree(id, ilk, wipeAmount = MDAI(0), freeAmount, {promise}) {
    ...
    return this.proxyActions[method](
        ...[this._managerAddress, this._adapterAddress(ilk),
            this._adapterAddress('DAI'), this.getIdBytes(id),
            freeAmount.toFixed(this._precision(freeAmount)),
            wipeAmount.toFixed('wei'), {dsProxy : true, promise}]
            .filter(x => x));
  }
  ...
  async wipe(id, wipeAmount, owner, {promise}) {
    ...
    return this.proxyActions.safeWipe(
        ...[this._managerAddress, this._adapterAddress('DAI'),
            this.getIdBytes(id), wipeAmount.toFixed('wei'), owner,
            {dsProxy : true, promise}]
            .filter(x => x));
  }
  ...
  unsafeWipe(id, wipeAmount, {promise}) {
    return this.proxyActions.wipe(
        ...[this._managerAddress, this._adapterAddress('DAI'),
            this.getIdBytes(id), wipeAmount.toFixed('wei'),
            {dsProxy : true, promise}]
            .filter(x => x));
  }
  ...
  async wipeAll(id, owner, {promise} = {}) {
    ...
    return this.proxyActions.safeWipeAll(
        this._managerAddress, this._adapterAddress('DAI'), this.getIdBytes(id),
        owner, {dsProxy : true, promise});
  }
  ...
  give(id, address, {promise}) {
    return this.proxyActions.give(this._managerAddress, this.getIdBytes(id),
                                  address, {dsProxy : true, promise});
  }
  ...
  giveToProxy(id, address, {promise}) {
    return this.proxyActions.giveToProxy(
        this._contractAddress('PROXY_REGISTRY'), this._managerAddress,
        this.getIdBytes(id), address, {dsProxy : true, promise});
  }
  ...
  getOwner(id) { return this._manager.owns(this.getIdBytes(id)); }
  ...
  get vat() { return this.get(SYSTEM_DATA).vat; }
  ...
  get _managerAddress() { return this._contractAddress('CDP_MANAGER'); }

  _contractAddress(name) {
    return this.get('smartContract').getContractAddress(name);
  }

  _adapterAddress(ilk) { return this.get(SYSTEM_DATA).adapterAddress(ilk); }
  ...
  getNewCdpId(txo) {
    ...
    const managerContract =
        this.get('smartContract').getContract('CDP_MANAGER');
    ...
    const receiptEvent =
        logs.filter(e => e.topics[0].toLowerCase() === topic.toLowerCase());
    const parsedLog =
        NewCdp.parse(receiptEvent[0].topics, receiptEvent[0].data);
  ...
  resetEventHistoryCache(id = null) {
    if (id !== null)
      delete this._eventHistoryCache[id];
    else
      this._eventHistoryCache = {};
  }
}
