...
import {ALLOWANCE_AMOUNT, defaultCdpTypes, DSR_DAI, MDAI, USD} from '../';
import {
...
export const collateralTypePrice = {
  generate : collateralTypeName => ({
    dependencies : [
      [ RATIO_DAI_USD ], [ PRICE_WITH_SAFETY_MARGIN, collateralTypeName ],
    ...
    computed : (ratioDaiUsd, priceWithSafetyMargin, liquidationRatio) => {
      ...
      const price = priceWithSafetyMargin.times(ratioDaiUsd.toNumber())
...
export const collateralTypesPrices = {
  generate : types => ({
    dependencies : () => types.map(
        collateralTypeName => [COLLATERAL_TYPE_PRICE, collateralTypeName]),
...
export const vaultExternalOwner = {
  generate : id => ({
    dependencies :
        [ [ PROXY_OWNER, [ VAULT_OWNER, id ] ], [ VAULT_OWNER, id ] ],
...
export const vaultCollateralAndDebt = {
  generate : (vaultType, vaultAddress) => ({
    ...
    computed : (encumberedCollateral,
                encumberedDebt) => [encumberedCollateral, encumberedDebt]
  })
};

// TODO This should also account for unencumbered collateral which is collateral
// on the join adapter
...
export const collateralValue = {
  generate : id => ({
    dependencies : [
      [ COLLATERAL_TYPE_PRICE, [ VAULT_TYPE, id ] ], [ COLLATERAL_AMOUNT, id ]
    ],
...
export const liquidationPrice = {
  generate : id => ({
    dependencies : [
      [ COLLATERAL_AMOUNT, id ], [ DEBT_VALUE, id ],
...
export const daiAvailable = {
  generate : id => ({
    dependencies : [
      [ COLLATERAL_VALUE, id ], [ DEBT_VALUE, id ],
      [ LIQUIDATION_RATIO, [ VAULT_TYPE, id ] ]
    ],
...
export const minSafeCollateralAmount = {
  generate : id => ({
    dependencies : [
      [ DEBT_VALUE, id ], [ LIQUIDATION_RATIO, [ VAULT_TYPE, id ] ],
...
export const collateralAvailableAmount = {
  generate : id => ({
    dependencies :
        [ [ COLLATERAL_AMOUNT, id ], [ MIN_SAFE_COLLATERAL_AMOUNT, id ] ],
    computed : (collateralAmount, minSafeCollateralAmount) => {
      if (minSafeCollateralAmount.toBigNumber().gt(
              collateralAmount.toBigNumber())) {
...
export const collateralTypeData = {
  generate : collateralTypeName => ({
    ...
    computed : (collateralTypePrice, annualStabilityFee, liquidationPenalty,
                liquidationRatio, priceWithSafetyMargin, debtFloor) => ({
      ...
      calculateCollateralizationRatio(collateralAmount, debtValue) {
        return calcCollateralizationRatio(
                   this.collateralTypePrice.times(collateralAmount), debtValue)
            .times(100)
            .toNumber();
      },
      calculateliquidationPrice(collateralAmount, debtValue) {
        return calcLiquidationPrice(collateralAmount, debtValue,
...
export const collateralTypesData = {
  generate : types => ({
    dependencies : () => types.map(
        collateralTypeName => [COLLATERAL_TYPE_DATA, collateralTypeName]),
...
export const vault = {
  generate : id => ({
    ...
    computed : (vaultType, vaultAddress, ownerAddress, externalOwnerAddress,
                encumberedCollateral, encumberedDebt, collateralTypePrice,
                debtValue, collateralizationRatio, collateralAmount,
                collateralValue, liquidationPrice, daiAvailable,
                collateralAvailableAmount, collateralAvailableValue,
                unlockedCollateral, liquidationRatio, liquidationPenalty,
                annualStabilityFee, debtFloor) => ({
      id : parseInt(id),
      vaultType,
      vaultAddress,
      ownerAddress,
      ...
      calculateLiquidationPrice({
        ...
        if (!collateralAmount || !debtValue || !liquidationRatio)
          return;
        return calcLiquidationPrice(collateralAmount, debtValue,
                                    liquidationRatio);
      },
  ...
  validate : {args : validateVaultId}
};

export const daiLockedInDsr = {
  generate : address => ({
    dependencies : [
      [ SAVINGS_DAI, [ PROXY_ADDRESS, address ] ], [ SAVINGS_RATE_ACCUMULATOR ]
    ],
...
export const savings = {
  generate : address => ({
    dependencies : [
      [ ANNUAL_DAI_SAVINGS_RATE ], [ DAI_SAVINGS_RATE ],
      [ DATE_EARNINGS_LAST_ACCRUED ], [ DAI_LOCKED_IN_DSR, address ],
      [ PROXY_ADDRESS, address ], [ SAVINGS_RATE_ACCUMULATOR ],
      [ SAVINGS_DAI, [ PROXY_ADDRESS, address ] ]
    ],
    computed :
        (annualDaiSavingsRate, daiSavingsRate, dateEarningsLastAccrued,
         daiLockedInDsr, proxyAddress, savingsRateAccumulator, savingsDai) => ({
  ...
  validate : {args : validateAddress`Invalid address for savings: ${'address'}`}
};

export const userVaultsList = {
  generate : address => ({
    dependencies : ({get}) => {
      const cdpManagerAddress =
          get('smartContract').getContractAddress('CDP_MANAGER');
      return [
...
export const collateralTypeCollateralization = {
  generate : (collateralTypeName, percentage = true) => ({
    ...
    computed : (debt, price, amount) => {
      ...
      const ratio =
          calcCollateralizationRatio(collateral, debt.toBigNumber()).times(100);
...
export const systemCollateralization = {
  generate : vaultTypes => ({
    dependencies : vaultTypes.map(
        vaultType => [COLLATERAL_TYPE_COLLATERALIZATION, vaultType, false]),
    computed : (...collateralizationValues) => {
      const {totalCollateralValue, totalDebtValue} =
          collateralizationValues.reduce(
              (acc, {collateralValue, debtValue}) => ({
                ...
                totalDebtValue :
                    acc.totalDebtValue.plus(debtValue.toBigNumber())
              }),
              {
      ...
      return calcCollateralizationRatio(totalCollateralValue, totalDebtValue)
          .times(100);
    }
  })
};
