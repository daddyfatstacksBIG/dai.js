...
beforeAll(async () => { maker = await mcdMaker({prefetch : false}); });

test('addConfig outputs contract addresses for all networks', () => {
  let {smartContract : {addContracts}, token : {erc20}} = McdPlugin.addConfig();

  // there are more addresses for testnet than other networks
  for (const contract of Object.values(addContracts)) {
    expect(contract.address).toEqual(expect.objectContaining({
  ...
  for (const token of erc20) {
    expect(token.address)
        .toEqual({testnet : expect.any(String), kovan : expect.any(String)});
  }
});

test('contract address mapping', async () => {
  const address =
      maker.service('smartContract').getContractAddressByName('PIP_ETH');
...
test('contract address overrides', async () => {
  ...
  const maker2 = await mcdMaker(
      {addressOverrides : {PIP_ETH : addr1, BAT : addr2}, prefetch : false});

  const scs = maker2.service('smartContract');
  expect(scs.getContractAddress('PIP_ETH')).toEqual(addr1);
  expect(() => { maker2.service('token').getToken('BAT'); }).not.toThrowError();
});
